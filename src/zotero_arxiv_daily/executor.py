from .enrichment import (
    ...
    deduplicate_papers,
    enrich_papers,
    prefilter_papers_with_code,
    parallel_filter,
    weighted_sort_papers,
...
def normalize_path_patterns(
    patterns: list[str] | ListConfig | None, config_key: str
) -> list[str] | None:
    if patterns is None:
        return None
...
class Executor:
    def __init__(self, config: DictConfig):
        ...
        self.include_path_patterns = normalize_path_patterns(
            config.zotero.include_path, "include_path"
        )
        self.ignore_path_patterns = normalize_path_patterns(
            config.zotero.ignore_path, "ignore_path"
        )
        self.retrievers = {
    ...
    def fetch_zotero_corpus(self) -> list[CorpusPaper]:
        ...
        if not user_id or not api_key:
            logger.warning(
                "Zotero credentials not configured. Skipping Zotero corpus fetch."
            )
            logger.warning(
                "Set ZOTERO_ID and ZOTERO_KEY environment variables to enable Zotero integration."
            )
            return []
    ...
    def filter_corpus(self, corpus: list[CorpusPaper]) -> list[CorpusPaper]:
        ...
        if self.ignore_path_patterns:
            logger.info(
                f"Excluding zotero papers matching ignore_path: {self.ignore_path_patterns}"
            )
            corpus = [
                c
                for c in corpus
    ...
    def run(self):
        ...
        if len(corpus) == 0:
            ...
            logger.warning(
                "To enable similarity matching, configure Zotero credentials."
            )
        ...
        # Multi-source integration: apply source weights
        source_weighting = self.config.executor.get("source_weighting", True)
        if source_weighting:
            logger.info("Applying source weights to papers...")
            for paper in all_papers:
                source_weight = paper.metadata.get("source_weight", 1.0)
                paper.metadata["source_weight"] = source_weight
        ...
        if len(all_papers) > 0:
            ...
            keyword_weight = float(
                self.config.executor.get("keyword_score_weight", 0.0)
            )
            similarity_threshold = float(
                self.config.executor.get("similarity_threshold", 2.0)
            )

            if filter_mode == "parallel":
                logger.info(
                    f"Using parallel filter mode (keyword OR similarity>={similarity_threshold})"
                )
                reranked_papers = apply_keyword_boost(
                    reranked_papers, keywords, keyword_weight
                )
                candidate_count = min(
                    len(reranked_papers),
                    self.config.executor.get("candidate_pool_size", 50),
                )
                reranked_papers = parallel_filter(
                    reranked_papers[:candidate_count], keywords, similarity_threshold
                )
            else:
                reranked_papers = apply_keyword_boost(
                    reranked_papers, keywords, keyword_weight
                )
                min_code_ratio = float(
                    self.config.executor.get("min_papers_with_code_ratio", 0.0)
                )
                if min_code_ratio > 0:
                    candidate_count = min(
                        len(reranked_papers),
                        self.config.executor.get("code_filter_candidates", 50),
                    )
                    reranked_papers = prefilter_papers_with_code(
                        reranked_papers[:candidate_count], min_code_ratio
                    )

            # Apply weighted sorting for multi-source papers
            if source_weighting:
                reranked_papers = weighted_sort_papers(reranked_papers)
        ...
        if source_errors:
            enrichment_failures["retrievers"] = [
        ...
        if self.config.reporting.enabled:
            ...
            markdown_path, json_path = write_outputs(
                reranked_papers, self.config.reporting, enrichment_failures
            )
        ...
        if self.config.email.get("enabled", False) and (
            reranked_papers or self.config.executor.send_empty
        ):
